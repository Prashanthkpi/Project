        ...
        function onRequest(context) {
            ...
            //     xml+='		<link name="NotoSansCJKsc" type="font" subtype="opentype" src="${nsfont.NotoSansCJKsc_Regular}" src-bold="${nsfont.NotoSansCJKsc_Bold}" bytes="2" />';
               ...
               xml+='        <macro id="nlheader">';
                ...
                xml+='	<td rowspan="3" style="padding: 0;"></td>';
                //<#if companyInformation.logoUrl?length != 0><img src="${companyInformation.logoUrl}" style="float: left; margin: 7px" /> </#if> ${companyInformation.companyName}<br />${companyInformation.addressText}</td>';
              ...
              //  xml+='		</#if>';
               ...
               xml+='    <table style="width: 100%; margin-top: 10px;"><tr>';
                xml+='	<td colspan="3" style="font-size: 8pt; padding: 6px 0 2px; font-weight: bold; color: #333333;">${record.billaddress}</td>';
                xml+='	<td colspan="3" style="font-size: 8pt; padding: 6px 0 2px; font-weight: bold; color: #333333;">${record.shipaddress}</td>';
            ...
            //  xml+='<#if record.item?has_content>';
              ...
              //<#list record.item as item><#if item_index==0>';
               xml+='<thead>';
                xml+='	<tr>';
                xml+='	<th align="center" colspan="3" style="padding: 10px 6px;">${item.quantity}</th>';
                xml+='	<th colspan="12" style="padding: 10px 6px;">${item.item}</th>';
                xml+='	<th align="right" colspan="4" style="padding: 10px 6px;">${item.rate}</th>';
                xml+='	<th align="right" colspan="4" style="padding: 10px 6px;">${item.amount}</th>';
                 xml+='	</tr>';
               xml+='</thead>';
            //    xml+='</#if>';
                xml+='<tr>';
            ...
            xml+='</table>';
            //     xml+='';
            //     xml+='<hr style="width: 100%; color: #d3d3d3; background-color: #d3d3d3; height: 1px;" />';
            //xml+='</#if>';
                xml+='<table style="page-break-inside: avoid; width: 100%; margin-top: 10px;"><tr>';
                xml+='	<td colspan="4">&nbsp;</td>';
               xml+='	<td align="right" style="font-weight: bold; color: #333333;">${record.subtotal}</td>';
